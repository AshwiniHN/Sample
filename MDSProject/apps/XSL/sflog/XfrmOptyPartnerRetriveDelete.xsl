...
<?oracle-xsl-mapper
  ...
  <!-- GENERATED BY ORACLE XSL MAPPER 11.1.1.7.8(build 150622.2350.0222) AT [THU MAR 03 12:05:11 CST 2016]. -->
?>
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <ns4:retrieveResponse>
      <xsl:for-each select="/ns3:SFLogObjectEvents/ns3:OpportunityPartnerDelete">
        <ns4:result>
          <ens:Id>
            <xsl:value-of select="substring-before(ns3:OBJECTID,'--')"/>
          </ens:Id>
		   <ens:Opportunity__c>
            <xsl:value-of select="substring-after(ns3:OBJECTID,'--')"/>
          </ens:Opportunity__c>
        </ns4:result>
      </xsl:for-each>
    </ns4:retrieveResponse>
