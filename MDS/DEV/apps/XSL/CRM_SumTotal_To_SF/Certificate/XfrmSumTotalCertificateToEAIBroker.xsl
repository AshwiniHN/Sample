	...
	<xsl:template match="/">
		<ns1:TxnMessage>
			...
			<ns1:ListOfDocument>
				<ns1:Document>
					<ns1:DocumentMessage>
						<xsl:copy-of select="/client:EAIBrokerProcessorProcessRequest/client:TxnMessage/tns:GetRosterDetailByTypeResult"/>
					</ns1:DocumentMessage>
				</ns1:Document>
			</ns1:ListOfDocument>
			<ns1:ListOfBlock>
				<xsl:for-each select="/client:EAIBrokerProcessorProcessRequest/client:TxnMessage/tns:GetRosterDetailByTypeResult/tns:ActAttemptCollection/tns:ActAttempt">
					<ns1:Block>
						<ns1:BlockType>
							<xsl:value-of select="/client:EAIBrokerProcessorProcessRequest/@ObjectType"/>
						</ns1:BlockType>
						<ns1:BlockKey>
							<xsl:value-of select="s2:Id"/>
						</ns1:BlockKey>
					</ns1:Block>
				</xsl:for-each>
			</ns1:ListOfBlock>
