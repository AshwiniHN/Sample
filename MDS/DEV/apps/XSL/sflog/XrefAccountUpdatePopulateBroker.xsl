      ...
      <db:InputParameters>
         <db:P_EVENT_DTLS>
            <xsl:for-each select="/tns:retrieveResponse/tns:result">
             <xsl:variable name="AccId" select="ens:Id"/>
             <xsl:if test="$AccId != ''">
               <db:P_EVENT_DTLS_ITEM>
                  ...
                  <xsl:for-each select="$sflog_objectevents/ns3:SFLogObjectEvents/ns3:accountUpdateEvents">
                     <xsl:if test="ns3:OBJECTID = $AccId">
                        <db:SFLOGID>
         ...
         <db:P_EVENT_BLOCK_DTLS>
            <xsl:for-each select="/tns:retrieveResponse/tns:result">
               <xsl:variable name="acc1Id" select="ens:Id"/>
               <xsl:variable name="ownerId" select="ens:OwnerId"/>
               <xsl:if test="$acc1Id != ''">
               <xsl:for-each select="$sflog_objectevents/ns3:SFLogObjectEvents/ns3:accountUpdateEvents">
                  <xsl:if test="ns3:OBJECTID = $acc1Id">
                     <xsl:variable name="SFLogId" select="ns3:SFLOGID"/>
                     <xsl:for-each select="for $i in 1 to 2 return $i">
                        <xsl:choose>
                           <xsl:when test="position() = 1 and $acc1Id != ''">
                           ...
                           <!--xsl:when test="position() = 2 and $ownerId != ''">
                              <db:P_EVENT_BLOCK_DTLS_ITEM>
                                 <db:SOURCE_TYPE>
                                    <xsl:text disable-output-escaping="no">CRM_SF_To_OP</xsl:text>
                                 </db:SOURCE_TYPE>
