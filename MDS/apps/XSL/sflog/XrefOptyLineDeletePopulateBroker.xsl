      ...
      <db:InputParameters>
         <db:P_EVENT_DTLS>
            <xsl:for-each select="/tns:retrieveResponse/tns:result">
               <xsl:variable name="OptyLineId" select="ens:Id"/>
               <xsl:if test="$OptyLineId != ''">
                  <db:P_EVENT_DTLS_ITEM>
                     ...
                     <xsl:for-each select="$sflog_objectevents/ns3:SFLogObjectEvents/ns3:optyLineDeleteEvents">
                        <xsl:if test="substring-before(ns3:OBJECTID,'--') = $OptyLineId">
                           <db:SFLOGID>
         ...
         <db:P_EVENT_BLOCK_DTLS>
            <xsl:for-each select="/tns:retrieveResponse/tns:result">
               <xsl:variable name="OptyLineId1" select="ens:Id"/>
               <xsl:variable name="OptyId" select="ens:OpportunityId"/>
               <xsl:if test="$OptyLineId1 != ''">
                  <xsl:for-each select="$sflog_objectevents/ns3:SFLogObjectEvents/ns3:optyLineDeleteEvents">
                     <xsl:if test="substring-before(ns3:OBJECTID,'--') = $OptyLineId1">
                        <xsl:variable name="SFLogId" select="ns3:SFLOGID"/>
                        <!-- Need only self Dependancy During Delete
                       db:P_EVENT_BLOCK_DTLS_ITEM>
                        ...
                        <db:P_EVENT_BLOCK_DTLS_ITEM>
                           ...
                           <db:OBJECT_FOREIGNKEY>
                              <xsl:value-of select="$OptyLineId1"/>
                           </db:OBJECT_FOREIGNKEY>
                           <db:OPERATION>
                              <xsl:text disable-output-escaping="no">insert</xsl:text>
                           </db:OPERATION>
